...
export const createContactUs = async (req: Request, res: Response) => {
  try {
    ...
    if (!process.env.GMAIL_EMAIL || !process.env.GMAIL_APP_PASSWORD) {
      console.error(
        "Nodemailer Error: GMAIL_EMAIL or GMAIL_APP_PASSWORD environment variables are not set."
      );
    ...
    const senderEmail = process.env.GMAIL_EMAIL as string;
    const receiverEmail = process.env.GMAIL_EMAIL as string; // Send to admin (your email)
